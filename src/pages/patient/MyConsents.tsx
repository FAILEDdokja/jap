...
function ApiMyConsents() {
  ...
  const ConsentRow = ({ consent, actions }: { consent: ApiConsent; actions?: React.ReactNode }) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-line p-3"><div><p className="text-[13px] font-semibold text-zinc-100">{consent.requesterName}</p><p className="text-[12px] text-zinc-400">{consent.purpose} · {consent.recordTypes.join(", ")}</p><p className="mt-0.5 text-[11px] text-zinc-500">Requested {fmtDate(consent.createdAt)} · valid until {consent.validUntil ? fmtDate(consent.validUntil) : "no end date"}</p></div><div className="flex items-center gap-2"><Badge tone={consent.status === "REJECTED" || consent.status === "REVOKED" ? "critical" : consent.status === "REQUESTED" ? "warning" : "positive"}>{consent.status.toLowerCase()}</Badge>{actions}</div></div>;
